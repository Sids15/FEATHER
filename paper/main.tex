...
while keeping the clean-stream false-alarm rate at its nominal budget (2\%
vs.\ 28\% for the ablation). These benchmarks produce output-visible drift
throughout, so they exercise FEATHER's tracking and false-alarm discipline;
to reach the silent regime we construct drift confined to the empirically
identified blind subspace of the trained ResNet-18, where a shift that is
invisible to every output statistic (softmax change below $10^{-10}$) drives a
synthetic accuracy from $99\%$ to $7\%$ and is caught by FEATHER on every
batch.
...
blind dimension, the same held-out bootstrap calibration at the 99th
percentile, and the same statistics; one projects onto the low-Fisher
subspace, the other onto the low-variance subspace of the activation
covariance. On the clean CIFAR-10 test stream --- where the calibrated
false-alarm budget should hold --- FEATHER alarms on $2.0 \pm 2.5\%$ of
batches, consistent with the ${\le}2\%$ nominal rate of two union-combined
99th-percentile tests, while the covariance monitor alarms on
$28.0 \pm 8.7\%$. On benign corruption episodes --- drifted, but harmless to
the model --- the covariance monitor alarms on $82.9 \pm 6.9\%$ of batches,
FEATHER on $47.6 \pm 6.9\%$; on gray and harmful episodes both alarm on
essentially every batch (Table~\ref{tab:alarms}). The benign gap is
$35.3 \pm 3.6$ percentage points, positive for every seed; an exact paired
sign-flip test across the five seeds gives $p = 0.0625$, the smallest value
the design admits. Low-variance directions are apparently easy for benign
corruptions --- and even for clean resampling --- to excite; low-Fisher
directions less so. The gap is the empirical content of the Fisher
weighting, and it answers the natural objection that the method is PCA in
disguise.

\begin{table}[!t]
\caption{Batch-level alarm rates on CIFAR-10-C under held-out calibration
(99th-percentile bootstrap thresholds; mean $\pm$ std over five seeds).
Both monitors catch every harmful batch; they differ on the clean stream
and the benign episodes a harm-aware monitor should ignore}
\label{tab:alarms}
\begin{tabular}{p{2.9cm}p{2.15cm}p{2.15cm}p{2.15cm}p{2.15cm}}
\hline\noalign{\smallskip}
Monitor & Clean & Benign & Gray & Harmful \\
\noalign{\smallskip}\svhline\noalign{\smallskip}
Covariance ablation & $0.28 \pm 0.09$ & $0.83 \pm 0.07$ & $1.00 \pm 0.00$ & $1.00 \pm 0.00$ \\
FEATHER & $\mathbf{0.02 \pm 0.02}$ & $\mathbf{0.48 \pm 0.07}$ & $1.00 \pm 0.00$ & $1.00 \pm 0.00$ \\
\noalign{\smallskip}\hline\noalign{\smallskip}
\end{tabular}
\end{table}
...
synthetic corruptions --- is, we think, the most valuable follow-up. Finally, held-out calibration keeps the clean
false-alarm rate at its budget only when the reference data matches the
deployment stream; where the two differ --- as for the MNIST CNN's
train-to-test activation shift --- thresholds must be recalibrated on
deployment-matched data (Sect.~\ref{sec:calibration}).
