...
% Pull top/bottom floats close to the surrounding text, so a figure that
% floats to the top of a column (e.g. Figure 1) does not leave a large gap
% above the next heading. Applied at begin-document so it wins over defaults.
\AtBeginDocument{\setlength{\textfloatsep}{6pt plus 1pt minus 1pt}}
...
together and at different speeds: sudden, gradual, incremental, or recurring.
The four speed profiles are drawn in Figure~\ref{fig:drift-types}. Tune a
detector to catch the sudden case and it can sit quietly through a slow one.
...
model's own outputs. Table~\ref{tab:families} places them side by side.
