...
keep the short proof of the blindness result, which is the one an operator
most needs to trust, and give the full rank-bound proof, the corollary on the
blind-subspace fraction, and the false-alarm analysis in
...
The supervised line begins with statistical process control \cite{page1954cusum}
and matures into stream-specific detectors. DDM tracks the online error rate
against its running minimum and warns, then alarms, as it climbs
\cite{gama2004learning}. EDDM instead watches the distance between successive
errors, which makes it more sensitive to slow, gradual drift
\cite{baena2006eddm}. ADWIN keeps a variable-length window and, whenever a
statistical test finds its two halves inconsistent, drops the stale half; its
guarantees follow from Hoeffding-style bounds \cite{bifet2007adwin}. These
methods are the natural upper bound for what a monitor can do \emph{with}
labels, which is exactly what streaming deployment withholds; we use them in
that role. River \cite{montiel2021river} provides reference implementations.

\paragraph{Input-distribution tests.}
Given batches $\mathcal{B}_t$ and a reference set, this family computes a
distance and thresholds it. Kernel two-sample tests such as MMD compare
embeddings through a characteristic kernel \cite{gretton2012kernel};
windowed Kolmogorov--Smirnov tests such as KSWIN act per feature
\cite{raab2020kswin}; black-box shift detection runs the test on the model's
own soft predictions or representations \cite{rabanser2019failing}; and
structured shifts such as label shift admit dedicated estimators
\cite{lipton2018label}. Because the distance is defined without reference to
the decision boundary, it cannot separate a boundary-parallel benign shift
from a boundary-crossing harmful one, which is the source of the alert fatigue
\cite{rabanser2019failing} that FEATHER is designed to avoid.

\paragraph{Output-based estimators.}
This family reads the softmax. MSP uses the maximum probability as a
correctness signal \cite{hendrycks2017baseline}; DoC tracks the gap between
current confidence and reference accuracy \cite{guillory2021doc}; ATC learns a
confidence threshold whose exceedance rate estimates accuracy
\cite{garg2022atc}; PUDD builds a prediction-uncertainty index that can move
before the error rate \cite{pudd2025}; and sequential error proxies test for
harmful shift directly \cite{amoukou2024sequential}. All inherit two problems:
deep-network miscalibration that worsens under shift
\cite{guo2017calibration, ovadia2019trust}, and the structural blindness of
Proposition~\ref{prop:blind}. FEATHER is complementary rather than competing:
it covers the blind region and defers output-visible drift to these tools.

\paragraph{Geometric and activation-space methods.}
Representation-space monitoring includes class-conditional Mahalanobis scoring
\cite{lee2018mahalanobis}, score-vector control charts \cite{zhang2023score},
null-direction probing of language-model representations \cite{pandey2025zdp},
and label-free activation monitors \cite{komorniczak2024parallel}. FEATHER
shares the representation-space setting but differs in what it weights each
direction by: task sensitivity through the Fisher matrix rather than variance
or density, which is what makes it harm-oriented, and it adds the calibrated
false-alarm control the others lack.
...
\paragraph{Synthetic testbed.}
Two Gaussian classes with means $\mu_0=(-1.5,0)$ and $\mu_1=(1.5,0)$ share the
covariance $\Sigma=\operatorname{diag}(1.0,0.25)$. With equal priors this makes
the Bayes-optimal boundary linear with normal $w=(3,0)$ through the class
midpoint. The output-visible direction is the normal $(1,0)$; the
output-invisible direction is the parallel $(0,1)$. Each stream is 30 batches
of 500 samples with drift injected at batch 15: a $4\sigma$ shift along the
invisible direction and a $3\sigma$ shift along the visible direction. The
monitor calibrates its 99th-percentile thresholds from 300 bootstrap
reference batches. Clean behavior is checked on 200 stationary batches.
...
coordination on the request path. This keeps monitoring centralized and
cheap while inference stays distributed. Validating this design at scale, and
measuring its overhead under real serving traffic, is left for future work.
