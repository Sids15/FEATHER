...
Section~\ref{sec:conclusion} concludes with limitations. The appendices give
the full proofs (Appendix~\ref{app:proofs}), extended related work
(Appendix~\ref{app:related}), the detailed experimental protocol
(Appendix~\ref{app:setup}), and the deployment design
...
Proposition~\ref{prop:fa}, proved in Appendix~\ref{app:proofs}, bounds the
...
on the request path. Appendix~\ref{app:deploy} gives the cost analysis and the
...
Two people can finish this in a semester. Every dataset is public, so nothing
...
This appendix gives the two results the main text states without full proof,
plus the corollary that quantifies the blind-subspace fraction.

\subsection{Fisher rank bound (Theorem~\ref{thm:rank})}
...
\subsection{Blind-subspace fraction}
...
\subsection{False-alarm control (Proposition~\ref{prop:fa})}
...
which we measure empirically.\hfill$\square$

\section{Extended Related Work}
\label{app:related}

This appendix expands Section~\ref{sec:survey} with more detail on each family
and where FEATHER sits relative to it.

\paragraph{Error-rate detectors.}
This line starts from statistical process control \cite{page1954cusum} and
grows into detectors built for streams. DDM compares the running error rate to
its own historical minimum, warning first and then alarming as the gap widens
\cite{gama2004learning}. EDDM looks instead at the spacing between successive
errors, which helps it catch drift that comes on slowly \cite{baena2006eddm}.
ADWIN holds a window that grows and shrinks; once a test says its two halves no
longer agree, it discards the older half, and its bounds come from Hoeffding
arguments \cite{bifet2007adwin}. What all three really give us is the best a
monitor could do if labels were free---which, in a stream, they are not. We
treat them as that upper bound, using River's implementations
\cite{montiel2021river}.

\paragraph{Input-distribution tests.}
Take a batch $\mathcal{B}_t$ and a reference set, measure a distance between
them, and threshold it---that is the whole recipe. Only the distance changes:
MMD compares embeddings through a characteristic kernel \cite{gretton2012kernel},
KSWIN runs a windowed Kolmogorov--Smirnov test feature by feature
\cite{raab2020kswin}, black-box shift detection applies the test to the model's
soft predictions or representations \cite{rabanser2019failing}, and label shift
gets its own estimators \cite{lipton2018label}. None of them consult the
decision boundary, so none can tell a boundary-parallel benign shift from a
boundary-crossing harmful one. That gap is where the alert fatigue
\cite{rabanser2019failing} comes from, and it is exactly what FEATHER sets out
to avoid.

\paragraph{Output-based estimators.}
Everything here reads the softmax. MSP takes the top probability as a
correctness signal \cite{hendrycks2017baseline}; DoC watches the gap between
today's confidence and the reference accuracy \cite{guillory2021doc}; ATC fits
a confidence threshold whose exceedance rate estimates accuracy
\cite{garg2022atc}; PUDD follows an uncertainty index that can turn before the
error rate does \cite{pudd2025}; and sequential error proxies go after harmful
shift head-on \cite{amoukou2024sequential}. Two problems run through all of
them: deep networks are badly calibrated and only get worse under shift
\cite{guo2017calibration, ovadia2019trust}, and then there is the structural
blindness of Proposition~\ref{prop:blind}. FEATHER is no rival to these. It
takes the blind region and leaves the output-visible drift to them.

\paragraph{Geometric and activation-space methods.}
Monitoring in representation space already takes several forms:
class-conditional Mahalanobis scoring \cite{lee2018mahalanobis}, score-vector
control charts \cite{zhang2023score}, null-direction probing of language-model
representations \cite{pandey2025zdp}, and label-free activation monitors
\cite{komorniczak2024parallel}. FEATHER lives in the same space, and the
weighting is where it parts from them. It ranks directions by task sensitivity,
through the Fisher matrix, rather than by variance or density---that is what
makes it harm-oriented---and it brings the calibrated false-alarm control the
others leave out.

\section{Experimental Protocol in Detail}
\label{app:setup}

This appendix records the exact settings behind Section~\ref{sec:setup} and
Section~\ref{sec:results}, so the runs can be reproduced.

\paragraph{Synthetic testbed.}
Two Gaussian classes, with means $\mu_0=(-1.5,0)$ and $\mu_1=(1.5,0)$, share a
covariance $\Sigma=\operatorname{diag}(1.0,0.25)$. Equal priors then place the
Bayes-optimal boundary on a straight line through the midpoint, with normal
$w=(3,0)$. That fixes the two directions we care about: the normal $(1,0)$ is
output-visible, and the parallel $(0,1)$ is output-invisible. A stream is 30
batches of 500 samples, and drift arrives at batch 15---$4\sigma$ along the
invisible direction in one run, $3\sigma$ along the visible one in the other.
Thresholds are taken at the 99th percentile from 300 bootstrap reference
batches, and 200 stationary batches serve as the clean-behavior check.

\paragraph{Harm labeling.}
Harm is measured, never assumed. Each episode is scored by the accuracy the
deployed model loses on held-out labels relative to the clean episode. In the
benchmark protocol an episode is benign below a 2-point drop and harmful above
a 10-point drop; the gray band between is dropped from detection scoring so a
borderline episode is never forced onto one side.

\paragraph{Metrics.}
We report AUROC for the harmful-versus-benign split with a rank-based
(Mann--Whitney) statistic; the false-alarm rate on measured-benign episodes at
matched calibration, which is the metric we lead with because it keeps
separating methods after AUROC saturates; the post-onset detection delay in
batches; and per-batch wall-clock cost against inference cost.

\paragraph{Baselines.}
The label-having upper bound is a River \cite{montiel2021river} error-rate
detector. The label-free output comparison is confidence- and entropy-based
statistics \cite{hendrycks2017baseline, garg2022atc}. The key ablation keeps
the entire FEATHER pipeline but replaces the Fisher matrix with activation
covariance at identical calibration, isolating the effect of task-weighting
versus density-weighting \cite{lee2018mahalanobis}.

\paragraph{Benchmarks.}
Beyond the synthetic testbed, the evaluation uses Rotated MNIST and CIFAR-10-C
\cite{hendrycks2019benchmarking} (19 corruptions $\times$ 5 severities, plus
clean), with models trained on clean data over several seeds so run-to-run
variation is reported. A natural WILDS shift \cite{koh2021wilds}, most likely
Camelyon17, is a stretch goal. Deep-benchmark numbers appear in the full
manuscript.

\section{Deployment as a Distributed Sidecar}
\label{app:deploy}

This appendix describes how FEATHER is meant to sit in a serving system. It is
